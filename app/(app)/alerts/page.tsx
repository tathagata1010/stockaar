    ...
    <PageRail>
      <RailSection label="Status" icon={<Bell className="h-3 w-3" />}>
        <div className="space-y-1.5">
          <div className="flex items-center justify-between rounded-lg border border-hairline bg-bg/40 px-3 py-2 text-xs">
            <span className="t-muted">Active</span>
            <span className="font-semibold t-num">{active}</span>
          </div>
          <div className="flex items-center justify-between rounded-lg border border-hairline bg-bg/40 px-3 py-2 text-xs">
            <span className="t-muted">Emailed (7d)</span>
            <span className="font-semibold t-num">{emailed7d ?? 0}</span>
          </div>
          <div className="flex items-center justify-between rounded-lg border border-hairline bg-bg/40 px-3 py-2 text-xs">
            <span className="t-muted">Limit</span>
            <span className="font-semibold t-num">{Number.isFinite(max) ? max : "∞"}</span>
          </div>
        </div>
      </RailSection>
      <RailSection label="Engine">
        ...
        <p className="mt-2 t-caption t-muted">
    ...
    <AppShell rail={rail} railLabel="Alerts filters">
      <section className="mesh-hero relative overflow-hidden rounded-3xl border border-hairline-strong bg-card/40 p-4 shadow-e4 sm:p-6 md:p-8 lg:p-10">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="rounded-md border border-hairline bg-card/60 px-4 py-3 text-sm">
            <div className="flex items-center gap-2 font-semibold"><Bell className="h-4 w-4 text-brand" /> Plan: {PLANS[plan].name}</div>
            <div className="mt-1 t-caption t-muted">{Number.isFinite(max) ? `${active}/${max} active` : `${active} active · unlimited`}</div>
          </div>
        </div>
      </section>
              ...
              <div key={n.id} className="surface p-3">
                <div className="flex items-center justify-between gap-2 text-xs">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="t-muted">{n.alerts?.exchange}</span>
                  </div>
                  <span className="t-caption t-muted">{timeAgo(n.sent_at)}</span>
                </div>
                {n.brief && (
                  <p className="mt-1.5 text-xs italic t-muted">{n.brief}</p>
                )}
              </div>
            ))}
          </div>
      ...
      <p className="mt-10 text-center t-caption t-muted">
        Checks run every 10 minutes. Limit of 1 email per stock per 6 hours.
      </p>
