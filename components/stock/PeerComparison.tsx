    ...
    <div className="space-y-3">
      <div className="surface hidden overflow-hidden md:block">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-hairline bg-bg/40 t-label">
...
function PeerTableRow({ row, isCurrent }: { row: PeerRow; isCurrent: boolean }) {
  ...
  return (
    <tr
      className={cn(
        "border-b border-hairline transition-colors duration-fast ease-out hover:bg-bg/30",
    ...
    >
      <td className="px-3 py-2.5 text-left">
        <Link href={`/stock/${entry.symbol}`} className="flex items-center gap-2 hover:text-brand">
          ...
          <div className="min-w-0">
            ...
            <div className="truncate t-caption t-muted">{entry.name}</div>
          </div>
        </Link>
      </td>
      ...
      <td className="px-3 py-2.5 text-right t-num">{pe ? formatNumber(pe, 1) : "—"}</td>
      <td className="px-3 py-2.5 text-right t-num">{roe != null ? formatPct(roe * 100) : "—"}</td>
      <td className="px-3 py-2.5 text-right t-num">{mcap ? formatCompactINR(mcap) : "—"}</td>
...
function PeerCard({ row, isCurrent }: { row: PeerRow; isCurrent: boolean }) {
  ...
  return (
    <Link
      ...
      className={cn(
        "surface block p-3 transition-colors duration-fast ease-out hover:border-brand/40",
    ...
    >
      ...
      <div className="mt-3">
        <div className="mb-1 flex items-center justify-between t-label">
          <span>52W Position</span>
          <span className="t-num">{rangePosition != null ? `${rangePosition.toFixed(0)}%` : "—"}</span>
        </div>
        <RangeBarMini value={rangePosition} />
      </div>
    </Link>
...
function MiniStat({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <div className="font-semibold t-num text-fg">{value}</div>
    </div>
  );
}

function RangeBarMini({ value }: { value: number | null }) {
  if (value == null) return <span className="t-muted">—</span>;
  const clamped = Math.max(0, Math.min(100, value));
  const color =
    clamped < 30 ? "bg-danger" :
    clamped > 70 ? "bg-accent" : "bg-warning";
  return (
    <div className="relative h-2 w-full overflow-hidden rounded-full bg-bg/60 ring-1 ring-hairline">
...
function ScoreDot({ score }: { score: number | null }) {
  if (score == null) return <span className="t-muted">—</span>;
  const color =
    score >= 70 ? "bg-accent text-bg" :
    score >= 50 ? "bg-warning text-bg" :
    "bg-danger text-bg";
  return (
    <span className={cn("inline-flex h-7 w-9 items-center justify-center rounded-full text-xs font-bold t-num", color)}>
      {score}
    </span>
  );
}
