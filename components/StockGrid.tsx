...
const SIGNAL_META = {
  POSITIVE: { label: "Positive", chip: "bg-accent/15 text-accent ring-accent/30", bar: "from-accent via-accent/70 to-brand",   icon: TrendingUp,   accent: "text-accent" },
  NEUTRAL:  { label: "Neutral",  chip: "bg-brand/15 text-brand ring-brand/30",    bar: "from-brand via-brand-2 to-accent",     icon: Activity,     accent: "text-brand"  },
  CAUTION:  { label: "Caution",  chip: "bg-danger/15 text-danger ring-danger/30", bar: "from-danger via-danger/70 to-warning", icon: TrendingDown, accent: "text-danger" },
} as const;
    ...
    <LazyChunks
      ...
      empty={
        <div className="surface flex flex-col items-center justify-center gap-2 p-12 text-center">
          ...
          <p className="t-body t-muted">{emptyText}</p>
        </div>
      }
    />
  );
    ...
    >
      ...
      <div className="relative">
        <div className="flex items-start justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <div className="truncate t-caption t-muted">{row.entry.name}</div>
            </div>
          </div>
        ...
        <div className="mt-4 flex items-baseline gap-2">
          <span className="num-display text-2xl font-bold t-num">{q ? formatINR(q.lastPrice) : "—"}</span>
          {q && (
            <span className={cn(
        ...
        {row.rangePosition != null && (
          <div className="mt-3">
            <div className="flex items-center justify-between t-label">
              <span>52W range</span>
              <span className="num-display font-bold t-num text-fg">{row.rangePosition.toFixed(0)}%</span>
            </div>
            <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-bg-2 ring-1 ring-hairline">
              <div
        ...
        {showScore && score != null && (
          <div className="mt-3">
            <div className="flex items-center justify-between t-label">
              ...
              <span className="num-display font-bold t-num text-fg">{score}<span className="t-muted">/100</span></span>
            </div>
        ...
        <div className="mt-4 flex items-center justify-between t-caption t-muted">
          <span className="t-label">{row.entry.sector}</span>
          <span className="inline-flex items-center gap-1 font-semibold transition-colors duration-fast ease-out group-hover:text-brand">
