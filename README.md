...
Indian stock intelligence platform — live NSE/BSE prices, scorecards, AI briefs, FII/DII flow tracking, screener, watchlist, alerts, daily newsletter.

**Stack:** Next.js 14 (App Router, RSC) · TypeScript · Tailwind · Supabase (Postgres + Auth + RLS) · Upstash Redis · Upstox + Yahoo Finance · Razorpay (paused) · Resend · NVIDIA NIM · Vercel.
...
- **Supabase** — https://supabase.com (URL + anon + service-role keys)
- **Upstash** — https://upstash.com (Redis REST URL + token)
- **Upstox Developer** — https://upstox.com/developer/ (OAuth; access token refreshed daily)
- **Resend** — https://resend.com (sending domain + API key)
- **NVIDIA NIM** — https://build.nvidia.com (free Llama 3.3 70B for AI brief)
- **Razorpay** — https://razorpay.com (currently paused; safe to skip until re-enabling paid plans)
...
![Stockaar system architecture](docs/images/Architecture-diagram.png)
...
For informational purposes only — not investment advice. The author is not a SEBI-registered Research Analyst.
